...
export async function POST(req: NextRequest) {
  try {
    const { entries, dealName } = await req.json() as {
      entries: Record<string, { rating: number; rationale: string }>;
      dealName?: string;
    };
    ...
    wsData.push(['Pillar', 'Sub-Factor ID', '평가 항목', '평가 기준 질문', '가중치(%)', '평점(1~3)', '점수(가중치×평점/100)', '현수준 기재']);
    ...
    for (const pid of pillars) {
      ...
      for (const item of items) {
        const entry = entries?.[item.id] ?? { rating: 2, rationale: '' };
        wsData.push([
          PILLAR_NAMES[pid],
          item.id,
          item.label,
    ...
    wsData.push(['총점', '', '', '', 100, '', { f: `G2+G${pillarStartRows.V}+G${pillarStartRows.D}+G${pillarStartRows.P}+G${pillarStartRows.E}` }, '']);
    ...
    const filename = dealName ? `${dealName}_signal_assessment.xlsx` : 'signal_assessment.xlsx';
    ...
    return NextResponse.json({ error: '엑셀 생성 오류' }, { status: 500 });
  }
}
