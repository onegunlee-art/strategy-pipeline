...
proposal_strategy는 위 전략 실행 경로의 액션을 기반으로 3~5개 작성하세요.`;
  ...
  const output = {
    ...
    // 수동 winning_points(2열) 우선, 없으면 기존 LLM string[]을 {customer_cfs:'', winning_point} 로 폴백
    winning_points: (Array.isArray(deal.winning_points) && deal.winning_points.length > 0)
      ? deal.winning_points
      : ((llmJson.winning_points as string[]) ?? []).map((w) => ({ customer_cfs: '', winning_point: w })),
