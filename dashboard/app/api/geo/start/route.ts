...
- hypothesis: 기사 분석에서 도출된 확률 결정 가설. FA·이코노미스트 기사를 "~에 따르면" 형식으로 직접 인용하며 1~2문장으로.
- strategy_low: 가능성이 낮을 때(<40%), 확률을 65%+ 로 끌어올릴 즉각 실행 전략. FA·이코노미스트 기사에서 발견한 미활용 기회를 구체적으로 인용하여 **반드시 5줄 이상** 작성. 첫 줄은 핵심 행동 요약, 이후 줄은 각각 FA·이코노미스트 기사 근거와 구체 실행방안. 행동 지향 어조 필수.
- strategy_mid: 가능성이 중간(40~65%)일 때, 80%+ 달성을 위한 핵심 레버. FA·이코노미스트 기사에서 가장 움직임이 큰 드라이버를 특정하여 **반드시 5줄 이상** 작성. 첫 줄은 핵심 레버 요약, 이후 줄은 각각 기사 근거·수치·구체 액션. 행동 지향 어조 필수.
- strategy_high: 가능성이 높을 때(>65%), 90%+ 확보를 위한 모멘텀 잠금 전략. FA·이코노미스트 기사에서 식별된 리스크를 사전 차단하는 구체적 액션으로 **반드시 5줄 이상** 작성. 첫 줄은 모멘텀 확보 핵심, 이후 줄은 잠재 리스크별 대응책. 행동 지향 어조 필수.
